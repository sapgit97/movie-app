...
export class MovieService {
  ...
  getFavourites():any{
    let search_url = `http://localhost:8098/api/v1/movies`;
    return this.http.get(search_url);
  }
  addFavourites(newFav: favourites):any{
    let post_url = `http://localhost:8098/api/v1/movie`;
    return this.http.post(post_url,newFav,httpOptions);
  }
  checkIfFav(id):any{
    let _url = `http://localhost:8098/api/v1/movie/${id}`;
  ...
  updateComments(favObj):any{
    let _url = `http://localhost:8098/api/v1/movie/${favObj.id}`;
